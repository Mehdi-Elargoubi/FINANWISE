...
export class StockRecommendationComponent {
  ...
  ajout: string | null = null; // pour strong buy et strong sell seulement
    ...
    this.finnhubService.getRecommendation(symbol).subscribe((data: Recommendation[]) => {
      this.recommendations = data;
        ...
        if (hold === Math.max(hold, buy, sell)) {
          ...
          this.ajout = `Strong Sell : ${latestRecommendation.strongSell}`;
          ...
          this.ajout = `Strong Buy : ${latestRecommendation.strongBuy}`;
        }
